...
const registrationValue=(item:Instrument)=>dateValue(String(item.dataRegistro||item.data_registro||''))
const currentInstrument=(items:Instrument[]|undefined)=>[...(items||[])].filter(i=>i.vigencia_status==='VIGENTE').sort((a,b)=>dateValue(b.vigencia)-dateValue(a.vigencia)||registrationValue(b)-registrationValue(a))[0]
...
export default function Home(){
 ...
 return <main className="shell"><header className="topbar"><div className="brand"><span className="brandMark">AD</span> Alerta Dissídio Zwei Bock</div><span className={unavailable?'sourceBadge offline':'sourceBadge'}>● Mediador / MTE — fonte oficial</span></header><div className="content">
