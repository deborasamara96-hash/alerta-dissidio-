...
export default function Home(){
 ...
 const unavailable=store?.overallStatus==='FONTE INDISPONÍVEL'; const consulted=store?.pairs?.filter(p=>p.status==='CONSULTADO')||[]; const instruments=consulted.reduce((n,p)=>n+(p.instrumentos?.length||0),0)
 ...
 <section className="grid"><div className="card"><div className="muted">Pares monitorados</div><div className="metric">12</div></div><div className="card"><div className="muted">Instrumentos encontrados</div><div className="metric">{store?instruments:'—'}</div></div><div className="card"><div className="muted">Pares consultados</div><div className="metric">{store?consulted.length:'—'}</div></div><div className="card"><div className="muted">Fonte indisponível</div><div className="metric">{store?store.pairs.filter(p=>p.status==='FONTE INDISPONÍVEL').length:'—'}</div></div><div className="card"><div className="status"><span className={`dot ${unavailable?'red':'warn'}`}/> Fonte</div><div className="metric" style={{fontSize:18}}>{unavailable?'FONTE INDISPONÍVEL':source?.status||'Aguardando monitoramento'}</div></div></section>
 ...
 <section className="section"><div className="sectionTitle"><h2>Sindicatos monitorados</h2><span className="muted">Histórico oficial armazenado no repositório</span></div><div className="table"><div className="row head"><div>#</div><div>Entidade patronal</div><div>Entidade laboral</div><div>Fonte</div><div>Situação / documento</div></div>{pairs.map(p=>{const r=store?.pairs?.find(x=>x.id===p[0]);const off=r?.status==='FONTE INDISPONÍVEL';const count=r?.instrumentos?.length||0;const first=r?.instrumentos?.[0];const url=first?documentUrl(first):'';return <div className="row" key={p[0]}><div className="pair">{p[0]}</div><div><strong>{p[3]}</strong><div className="cnpj">CNPJ {p[1]}</div></div><div><strong>{p[4]}</strong><div className="cnpj">CNPJ {p[2]}</div></div><div><span className="tag">Mediador/MTE</span></div><div><span className={`tag ${off?'offline':r?'':'pending'}`}>{off?'FONTE INDISPONÍVEL':r?`${count} instrumento(s) validado(s)`:'Aguardando execução automática'}</span>{r&&!off&&url&&<div style={{marginTop:8}}><a href={url} target="_blank" rel="noopener noreferrer" className="tag" style={{textDecoration:'none',cursor:'pointer'}}>📄 Abrir documento oficial</a></div>}</div></div>})}</div></section><div className="footer">V3 • Consulta manual real + monitoramento automático diário. Sem dados fictícios.</div></div></main>
}
