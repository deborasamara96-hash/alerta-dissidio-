...
function extractSolicitations(html) {
  ...
  const found = new Set()
  for (const re of [/MR\s*\d+\s*(?:%2F|%2f|\/|\\/)\s*\d{4}/gi]) {
...
async function openOfficialSession(page) {
  ...
  if (!/Consultar Instrumentos Coletivos Registrados/i.test(body)) {
...
async function searchByCnpj(page, cnpj) {
  ...
  // O Mediador mantém o formulário oficial e recebe a pesquisa por POST/XHR.
  // Enviamos os mesmos campos dentro da sessão do navegador, preservando cookies.
  const result = await page.evaluate(async ({ endpoint, cnpj }) => {
    ...
    ]) params.append('tpRequerimento', value)
  ...
  const noResults = /nenhum|não foram encontrados|nao foram encontrados|nenhum registro|0\s+registro/.test(lower)
  return {
      ...
      try {
        ...
        if (discovery.successfulQueries === 0) {
          results.push({
            ...
            error: discovery.error || 'Não foi possível obter resposta da consulta oficial.',
            instrumentos: [], candidatosVerificados: 0,
        ...
        results.push({
          id, patronal, laboral,
          status: 'CONSULTADO',
          metodo: 'POST oficial do Mediador dentro de sessão autenticada + validação dos dois CNPJs no documento oficial',
          instrumentos: unique,
